...
class BettingEnv(gym.Env):
    ...
    def calculate_reward(self, action, bet_on, game_result, bet_amount, odds):
        if self.bet_successful(bet_on, game_result):
            ...
            #   ********** DEBUG ***************
            if self.printdbg:
                print("WIN  ", "Action : ", bet_on , "   Result : ", game_result, "  Return : ",  round(bet_amount * (odds[action]), 2), "  Odds : ", odds)
            #return 1
            if bet_on == 'H':
                self.home_Wins+=1
                if self.printdbg:
                    print("Reward for H: ", 1 -  (1 / (odds[action]))) # (1 / odds[action]) * (1 / 0.46))
                ...
                self.draws+=1
                if self.printdbg:
                    print("Reward for D: ",  (1 - (1 /odds[action])) * 1.85)#((1 / odds[action])) * (1 / 0.24))
                ...
                self.away_Wins+=1
                if self.printdbg:    
                    print("Reward for A: ", (1 - (1 /odds[action])) * 1.54) # ((1 / odds[action])) * (1 / 0.3))
            ...
            if self.printdbg:
                print("LOSE  ", "Action : ", bet_on , "   Result : ", game_result, "  Return : ", 0 - bet_amount, "  Odds : ", odds)

            '''
            if action == 0:
                return 0 - ((1 / odds[1]) + (1 / odds[2]))
            elif action == 1:
                return 0 - ((1 / odds[0]) + (1 / odds[2]))
            else:
                return 0 - ((1 / odds[0]) + (1 / odds[1]))
            '''
            holder = -bet_amount
            ...
            if bet_on == 'H':
                if self.printdbg:
                    print("Minus Reward for H: ", 1 - odds[action])
                self.home_Wins+=1
                #return 0
                #return 1 - odds[action]
                ...
                if self.printdbg:
                    print("Minus Reward for D: ", 1 - odds[action])
                self.draws+=1
                #return 0
                return 0 - (1 / odds[action])
                return 1 - odds[action]
                ...
                if self.printdbg:
                    print("Minus Reward for A: ", 1 - odds[action])
                self.away_Wins+=1
                #return 0
                return 0 - (1 / odds[action])
                return 1 - odds[action]
