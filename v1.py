...
def update_home_win_streak(row, home_win_streaks):
    home_team = row['HomeTeam']
    result = row['FTR']

    if result == 'H':  # Home team won
        home_win_streaks[home_team] = 0  # Reset the streak
    else:
        # Increment the streak if the game was a home game for the team
        if home_team in home_win_streaks:
            home_win_streaks[home_team] += 1
...
def kegs_System(row, teamVal, league_table, current_non_win_streaks, totalTrades):
    ...
    if(league_table[tName]['Wins'] <= (len(league_table) - league_table[tName]['Position'])):
        if(league_table[tName]['Matches Played'] >= 5):
            if(league_table[tName]['Position'] < (len(league_table) / 2)):
                ...
                #bet_amount = 10 * 2 ** current_non_win_streaks[tName]
    ...
    if(league_table[tName]['Losses'] < league_table[tName]['Position']):
        if(league_table[tName]['Matches Played'] >= 5):
            if(league_table[tName]['Position'] > (len(league_table) / 2)):
                #init pot
                #VERY IMPORTANT, the 10 is base amount, 2 is the multiplier
...
league_table = {}
betting_results = []
home_win_streaks = {team: 0 for team in pd.concat([e0_data['HomeTeam'], e0_data['AwayTeam']]).unique()}
...
for index, row in e0_data.iterrows():
    ...
    totalTrades = kegs_System(row, 0, league_table, current_non_win_streaks, totalTrades)
    totalTrades = kegs_System(row, 1, league_table, current_non_win_streaks, totalTrades)
    #update_draw_streak(row, draw_streaks)
    update_non_draw_streak(row, draw_streaks, longest_non_draw_streaks)
    update_home_win_streak(row, home_win_streaks)
