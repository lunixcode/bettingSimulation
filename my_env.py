...
class BettingEnv(gym.Env):
    ...
    #def step(self, action):
        ...
        if action == 0:  # Bet on Team A
            odds = row['B365H']
            reward = self.calculate_reward(bet_on='H' , game_result=game_result, bet_amount = bet_amount,  odds = odds)
        elif action == 1:  # Bet on Draw
            odds = row['B365D']
            reward = self.calculate_reward(bet_on='D', game_result=game_result,  bet_amount = bet_amount,  odds = odds)
        elif action == 2:  # Bet on Team B
            odds = row['B365A']
            reward = self.calculate_reward(bet_on='A', game_result=game_result,  bet_amount = bet_amount,  odds = odds)
        else:  # No bet
            reward = self.calculate_reward(bet_on=None, game_result=game_result,  bet_amount = bet_amount,  odds = odds )
    ...
    def calculate_reward(self,bet_on, game_result, bet_amount, odds):
        if self.bet_successful(bet_on, game_result):
            return bet_amount * (odds)# Profit from the bet
        else:
            return -bet_amount  # No bet or a draw, for instance
