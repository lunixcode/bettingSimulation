...
class BettingEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.printDBG = False
        self.writeDBG = True
        self.file = open('debug_file.txt', 'a')
        ...
        # Define the observation space (only form variables)
        self.observation_space = spaces.Box(
            low=np.array([0] * num_form_values),  # Only values for form variables
            high=np.array([1] * num_form_values),
            dtype=np.float32
        )
    ...
    #def step(self, action):
        ...
        # Get the result of the current game
            ...
            row1 = self.e0_data.iloc[cIndex+1]
        ...
        #  *************** DEBUG *****************
        if self.writeDBG:
            self.file.write('Home Team :' + row['HomeTeam'] + '  Away Team : ' + row['AwayTeam'] + '\n')
        if self.printDBG == True:
    ...
    def calculate_reward(self, action, bet_on, game_result, bet_amount, odds):
        ...
        if self.writeDBG:
            self.file.write(str(self.get_state()) + '\n')
        if self.printDBG == True:
            self.print_state()
        ...
        if self.bet_successful(bet_on, game_result):
            ...
            #   ********** DEBUG ***************
            if self.writeDBG: 
                self.file.write("WIN  "+ "Action : " + str(bet_on) + "   Result : "+ str(game_result)+ "  Return : "+  str(round(bet_amount * (odds[action]), 2)) + "  Total: " + str(self.running_total) + "  Odds : " + str(odds) + '\n' )
            if self.printDBG:
            ...
            if bet_on == 'H':
                self.home_Wins+=1
                if self.writeDBG:
                    self.file.write("Reward for H: " + str(1 -  (1 / (odds[action]))) + '\n\n')
                if self.printDBG:
                ...
                self.draws+=1
                if self.writeDBG:
                    self.file.write('Reward for D: '+  str((1 - (1 /odds[action]))) + '\n\n')
                if self.printDBG:
                ...
                self.away_Wins+=1
                if self.writeDBG:
                    self.file.write("Reward for A: " + str((1 - (1 /odds[action]))) + "\n\n")
                if self.printDBG:
            ...
            self.running_total -=  bet_amount
            if self.writeDBG:
                self.file.write('LOSE  ' + 'Action : ' + str(bet_on) + '   Result : ' + str(game_result) + '  Return : ' + str(0 - bet_amount) + '  Total: ' + str(self.running_total) + '  Odds : ' + str(odds) + '\n')
            if self.printDBG:
            ...
            if bet_on == 'H':
                if self.writeDBG:
                    self.file.write('Minus Reward for H: ' + str(0 - (1 / odds[action])) + '\n\n')
                if self.printDBG:
                ...
                if self.writeDBG:
                    self.file.write('Minus Reward for D: ' + str(0 - (1 / odds[action])) + '\n\n')
                if self.printDBG:
                ...
                if self.writeDBG: 
                    self.file.write('Minus Reward for A: ' + str(0 - (1 / odds[action])) + '\n\n')
                if self.printDBG:
    ...
    def reset(self):
        ...
        self.state = np.array([home_form, away_form, hForm, aForm])
        self.countForEpisode = 0
    ...
    def update_state(self, row):
        ...
        self.state = np.array([homeForm, awayForm, hForm, aForm])
    ...
    def __del__(self):
        self.file.close()  # Close in the destructor (optional)
